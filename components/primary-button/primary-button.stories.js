...
storiesOf('Primary Button', module)
  ...
  .add('Regular with changed color', () => (
    <PrimaryButton onClick={action('clicked')} color="red" backgroundColor="black">Hello Button</PrimaryButton>
  ))
  .add('Disabled', () => (
    <PrimaryButton onClick={action('clicked')} disabled={true}>Hello Button</PrimaryButton>
  ))